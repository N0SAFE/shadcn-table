...
> = {
  value: T;
  getFilterTypeDef: (type: keyof T) => FilterTypeDef<Data, Meta>;
};
...
export interface FilterConfig<T extends FilterAdapter> {
  ...
  label: string;
  meta?: T extends FilterAdapter<any, infer Meta> ? Meta : never;
...
export type FiltersActions<T extends FilterAdapter = FilterAdapter> = {
  generateFilter: (
    filter: Partial<Omit<FilterConfig<T>, "type" | "id">> &
      Pick<FilterConfig<T>, "type" | "id">
  ) => Filter<T>;
};
